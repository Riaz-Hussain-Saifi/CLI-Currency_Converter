...
async function main() {
    ...
    while (tryAgain) {
        ...
        const fromCountry = await selectCountry("Currency convert from: \t", countryChoices);
        const toCountry = await selectCountry("Currency convert to: \t", countryChoices);
        ...
        `\t\n ${amount} ${Chalk.blue(fromCountry)} is equal to ${Chalk.blue(convertedAmount.toFixed(2))} ${toCountry} \n`));
